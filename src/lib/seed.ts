/**
 * Reusable seed logic. Callable from the CLI (`npm run db:seed`) or from
 * the browser via /api/setup — so a Vercel-only user can bootstrap their
 * database without ever opening a terminal.
 */
...
export async function seedDatabase(): Promise<{ cards: number; sales: number; listings: number }> {
  ...
  return { cards, sales: salesCount, listings };
}
