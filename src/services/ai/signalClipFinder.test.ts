...
describe("findClips routing", () => {
  ...
  /** A placeholder transcript that happens to span most of a short source. */
  const longPlaceholder: Transcript = {
    source: "mock",
    language: "en",
  ...
  it("uses a placeholder transcript only when told it matches the source", () => {
    // The demo generator emits footage and transcript together, so its words
    // should drive scoring rather than being discarded.
    const clips = findClips(longPlaceholder, settings, {
      duration: 90,
      placeholderMatchesSource: true,
    });
    expect(clips.length).toBeGreaterThan(0);
  ...
  it("distrusts a placeholder that merely spans most of a short upload", () => {
    // A real 2-minute upload carrying the demo transcript: the text covers 75%
    // of the runtime, which a coverage heuristic would wrongly accept and then
    // caption the video with words nobody said.
    const clips = findClips(longPlaceholder, settings, {
      duration: 120,
      peaks: peaksWithBurstsAt([40], 120, 120),
      peaksDuration: 120,
    });
    expect(clips.length).toBeGreaterThan(0);
    expect(clips.every((c) => c.title.startsWith("HIGH ENERGY"))).toBe(true);
